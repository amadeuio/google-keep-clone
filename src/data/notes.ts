import { NOTE_COLORS } from '@/constants';
import type { Note } from '@/types';
import { v4 as uuidv4 } from 'uuid';

export const notes: Note[] = [
  {
    id: uuidv4(),
    title: 'Welcome to Google Keep Clone',
    content:
      'This is your first note! You can edit, delete, and create new notes. Try clicking on this note to edit it.',
    createdAt: new Date('2024-01-15T10:00:00Z'),
    updatedAt: new Date('2024-01-15T10:00:00Z'),
    color: NOTE_COLORS.YELLOW,
    labels: [],
    isArchived: false,
    isPinned: true,
  },
  {
    id: uuidv4(),
    title: 'Shopping List',
    content: '• Milk\n• Bread\n• Eggs\n• Bananas\n• Coffee\n• Toilet paper',
    createdAt: new Date('2024-01-14T15:30:00Z'),
    updatedAt: new Date('2024-01-14T16:45:00Z'),
    color: NOTE_COLORS.GREEN,
    labels: ['shopping', 'groceries'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Meeting Notes - Project Kickoff',
    content:
      'Team meeting scheduled for Friday 2 PM\n\nAgenda:\n- Project timeline review\n- Resource allocation\n- Next steps\n\nAction items:\n- Send meeting invite\n- Prepare presentation slides',
    createdAt: new Date('2024-01-13T09:15:00Z'),
    updatedAt: new Date('2024-01-13T11:20:00Z'),
    color: NOTE_COLORS.BLUE,
    labels: ['work', 'meeting'],
    isArchived: false,
    isPinned: true,
  },
  {
    id: uuidv4(),
    title: 'Recipe: Chocolate Chip Cookies',
    content:
      'Ingredients:\n- 2 1/4 cups flour\n- 1 cup butter\n- 3/4 cup sugar\n- 3/4 cup brown sugar\n- 2 eggs\n- 1 tsp vanilla\n- 1 tsp baking soda\n- 1/2 tsp salt\n- 2 cups chocolate chips\n\nInstructions:\n1. Preheat oven to 375°F\n2. Mix ingredients\n3. Bake for 9-11 minutes',
    createdAt: new Date('2024-01-12T14:20:00Z'),
    updatedAt: new Date('2024-01-12T14:20:00Z'),
    color: NOTE_COLORS.RED,
    labels: ['recipe', 'baking'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Ideas for Weekend',
    content:
      'Things to do this weekend:\n\n• Visit the new art museum\n• Try that new restaurant downtown\n• Go for a hike at the park\n• Watch the new movie everyone is talking about\n• Call mom and dad\n• Clean the apartment',
    createdAt: new Date('2024-01-11T18:45:00Z'),
    updatedAt: new Date('2024-01-11T18:45:00Z'),
    color: NOTE_COLORS.PURPLE,
    labels: ['personal', 'weekend'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Quick Reminder',
    content:
      "Don't forget to:\n- Pay electricity bill\n- Schedule dentist appointment\n- Return library books",
    createdAt: new Date('2024-01-10T12:00:00Z'),
    updatedAt: new Date('2024-01-10T12:00:00Z'),
    color: NOTE_COLORS.TEAL,
    labels: ['reminders'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Book Recommendations',
    content:
      'Books to read:\n- Atomic Habits by James Clear\n- The Pragmatic Programmer\n- Deep Work by Cal Newport\n- The Design of Everyday Things',
    createdAt: new Date('2024-01-09T09:00:00Z'),
    updatedAt: new Date('2024-01-09T09:00:00Z'),
    color: NOTE_COLORS.ORANGE,
    labels: ['books', 'reading'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Vacation Planning',
    content:
      'Destinations:\n- Kyoto, Japan\n- Barcelona, Spain\n- Banff, Canada\n\nTo do:\n- Check passport validity\n- Book flights\n- Reserve hotels',
    createdAt: new Date('2024-01-08T17:30:00Z'),
    updatedAt: new Date('2024-01-08T17:30:00Z'),
    color: NOTE_COLORS.PINK,
    labels: ['travel', 'planning'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Work Tasks',
    content:
      '- Finish quarterly report\n- Review pull requests\n- Schedule 1:1 with team\n- Update project documentation',
    createdAt: new Date('2024-01-07T13:10:00Z'),
    updatedAt: new Date('2024-01-07T13:10:00Z'),
    color: NOTE_COLORS.GRAY,
    labels: ['work', 'tasks'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Fitness Goals',
    content:
      'Weekly goals:\n- Run 10 miles\n- 3 strength workouts\n- 1 yoga session\n- Drink 2L water daily',
    createdAt: new Date('2024-01-06T08:00:00Z'),
    updatedAt: new Date('2024-01-06T08:00:00Z'),
    color: NOTE_COLORS.GREEN,
    labels: ['fitness', 'health'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Gift Ideas',
    content:
      'For Sarah:\n- Scented candles\n- Art supplies\n- Cookbook\n\nFor Mike:\n- Wireless earbuds\n- Hiking backpack\n- Board game',
    createdAt: new Date('2024-01-05T19:45:00Z'),
    updatedAt: new Date('2024-01-05T19:45:00Z'),
    color: NOTE_COLORS.BROWN,
    labels: ['gifts', 'ideas'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Daily Journal',
    content:
      '2024-01-16\nToday I started using this app. Looking forward to organizing my thoughts!',
    createdAt: new Date('2024-01-16T21:00:00Z'),
    updatedAt: new Date('2024-01-16T21:00:00Z'),
    color: NOTE_COLORS.DEFAULT,
    labels: ['journal'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Movies to Watch',
    content:
      '- Inception\n- Parasite\n- The Grand Budapest Hotel\n- Everything Everywhere All at Once',
    createdAt: new Date('2024-01-04T20:00:00Z'),
    updatedAt: new Date('2024-01-04T20:00:00Z'),
    color: NOTE_COLORS.GRAY,
    labels: ['movies', 'entertainment'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Birthday Party Checklist',
    content:
      '- Send invitations\n- Order cake\n- Buy decorations\n- Plan games\n- Confirm venue booking',
    createdAt: new Date('2024-01-03T10:30:00Z'),
    updatedAt: new Date('2024-01-03T10:30:00Z'),
    color: NOTE_COLORS.YELLOW,
    labels: ['party', 'checklist'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Learning Goals',
    content:
      '- Finish React course\n- Practice TypeScript\n- Read about accessibility in web design',
    createdAt: new Date('2024-01-02T09:00:00Z'),
    updatedAt: new Date('2024-01-02T09:00:00Z'),
    color: NOTE_COLORS.BLUE,
    labels: ['learning', 'goals'],
    isArchived: false,
    isPinned: false,
  },
  {
    id: uuidv4(),
    title: 'Packing List for Trip',
    content:
      '- Passport\n- Tickets\n- Clothes\n- Toiletries\n- Camera\n- Chargers\n- Travel guidebook',
    createdAt: new Date('2024-01-01T15:00:00Z'),
    updatedAt: new Date('2024-01-01T15:00:00Z'),
    color: NOTE_COLORS.PINK,
    labels: ['travel', 'packing'],
    isArchived: false,
    isPinned: false,
  },
];
